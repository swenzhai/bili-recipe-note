...
“烹饪技巧”是与菜谱库分开的只读专栏，只展示从视频中沉淀的通用技巧、原理、适用场景、标签和来源证据。可按关键词、技巧分类和更新时间筛选查看；提取、确认、编辑、去重及同步操作仍集中在“知识库”页面，不会混入菜谱或点餐流程。

知识库也支持上传可复制文字的 PDF、菜谱截图/书页图片，或外部 OCR 导出的 TXT/Markdown。图片会先通过本机 Tesseract OCR 转成文字，再自动提取通用技巧，并保留图片来源和 OCR 原文摘录。专栏默认只显示已确认条目；候选内容、质量提醒、复习记录和版本恢复在“知识库”页面处理。

图片 OCR 需要本机安装 Tesseract 及 `chi_sim` 中文语言包；若不方便安装，也可以先用手机或其他工具 OCR 后上传 TXT。
...
部署包和批次交接包会同时携带 `.bili-recipe-notes/knowledge_base.json`，部署包还会携带固定备份，保证迁移电脑时不会丢失个人烹饪技巧。
