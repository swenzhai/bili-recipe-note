...
项目同时保留两种手机使用方式：`mobile/` 是原有 Flutter 离线客户端，`web/` 是不需要 Apple 开发者账号、可添加到 iPhone 主屏幕的 PWA。Mac 端仍负责生成和管理菜谱；网页版通过导入菜谱包离线搜索、点菜、生成采购清单和分步烹饪，原生 App 的局域网同步继续可用。开发说明见 [`mobile/README.md`](mobile/README.md) 和 [`web/README.md`](web/README.md)。
...
- 在手机客户端选择“本餐”菜品、调整份量和口味备注，自动汇总采购清单并连续进入烹饪模式
- 管理局域网手机客户端配对、设备撤销、实践日志和同步冲突
- 为 PWA 导出离线菜谱包，可选择全部步骤图、每道菜仅一张图或只导出文字；网页版数据只保存在手机浏览器中
...
- 步骤图多候选优选、精确重截、上传替代图或明确不配图；低质量候选不会被强行保存
- 显示菜谱质量评分、问题和改进建议
